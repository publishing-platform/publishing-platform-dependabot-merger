...
class PullRequest
  ...
  def validate_single_commit
    commits = GitHubClient.instance.pull_request_commits("publishing-platform/#{@api_response.base.repo.name}", @api_response.number)
    commits.count == 1
  end

  def validate_files_changed
    commit = GitHubClient.instance.commit("publishing-platform/#{@api_response.base.repo.name}", @api_response.head.sha)
  ...
  def validate_ci_passes
    ...
    jobs_url = "https://api.github.com/repos/publishing-platform/#{@api_response.base.repo.name}/actions/runs/#{ci_workflow_run_id}/jobs"
  ...
  def approve!
    approval_message = <<~REVIEW_COMMENT
      This PR has been scanned and automatically approved by [publishing-platform-dependabot-merger](https://github.com/publishing-platform/publishing-platform-dependabot-merger).
    REVIEW_COMMENT
    response = GitHubClient.post(
      "https://api.github.com/repos/publishing-platform/#{@api_response.base.repo.name}/pulls/#{@api_response.number}/reviews",
      {
        event: "APPROVE",
  ...
  def merge!
    GitHubClient.instance.merge_pull_request("publishing-platform/#{@api_response.base.repo.name}", @api_response.number)
  rescue Octokit::Error => e
    puts "Error merging pull request: #{e.message}"
  end

  def head_commit
    @head_commit ||= GitHubClient.instance.commit("publishing-platform/#{@api_response.base.repo.name}", @api_response.head.sha).commit
  end
...
private
  ...
  def ci_workflow_run_id
    @ci_workflow_run_id ||= begin
      # No method exists for this in Octokit,
      # so we need to make the API call manually.
      uri = "https://api.github.com/repos/publishing-platform/#{@api_response.base.repo.name}/actions/runs?head_sha=#{@api_response.head.sha}"

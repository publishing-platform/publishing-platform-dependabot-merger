...
module AutoMerger
  ...
  def self.merge_dependabot_prs(dry_run: false)
    Repo.all.each do |repo|
      policy_manager = PolicyManager.new(repo.publishing_platform_dependabot_merger_config)

      if !policy_manager.remote_config_exists?
        puts "#{repo.name}: the remote .publishing_platform_dependabot_merger.yml file is missing."
      elsif !policy_manager.valid_remote_config_syntax?
        puts "#{repo.name}: the remote .publishing_platform_dependabot_merger.yml YAML syntax is corrupt."
      elsif !policy_manager.remote_config_api_version_supported?
        puts "#{repo.name}: the remote .publishing_platform_dependabot_merger.yml file is using an unsupported API version."
  ...
  def self.analyse_dependabot_pr(url)
    puts "Analysing #{url}..."
    _, repo_name, pr_number = url.match(/publishing-platform\/(.+)\/pull\/(.+)$/).to_a

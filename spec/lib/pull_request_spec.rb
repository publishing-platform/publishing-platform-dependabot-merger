...
RSpec.describe PullRequest do
  ...
  def single_dependency_commit
    <<~TEXT
      Bump publishing_platform_publishing_components from 35.7.0 to 35.8.0

      Bumps [publishing_platform_publishing_components](https://github.com/publishing-platform/publishing_platform_publishing_components) from 35.7.0 to 35.8.0.
      - [Changelog](https://github.com/publishing-platform/publishing_platform_publishing_components/blob/main/CHANGELOG.md)
      - [Commits](publishing-platform/publishing_platform_publishing_components@v35.7.0...v35.8.0)

      ---
      updated-dependencies:
      - dependency-name: publishing_platform_publishing_components
  ...
  let(:pull_request_api_response) do
    ...
    # just like OctoKit
    OpenStruct.new(
      url: "https://api.github.com/repos/publishing-platform/#{repo_name}/pulls/1",
      number: 1,
      state: "open",
      title: "First PR",
      user: OpenStruct.new(
      ...
      labels: [],
      draft: false,
      statuses_url: "https://api.github.com/repos/publishing-platform/#{repo_name}/statuses/#{sha}",
      head: OpenStruct.new(
        sha:,
      ),
      base: OpenStruct.new(
  ...
  let(:head_commit_api_url) { "https://api.github.com/repos/publishing-platform/#{repo_name}/commits/#{sha}" }
      ...
      files: [
        {
          ...
          patch: <<~GEMFILE_LOCK_DIFF,
            publishing_platform_markdown (0.13.0)
                    kramdown (>= 2.3.1, < 2.5.2)
                    sanitize (>= 6, < 8)
            -    publishing_platform_publishing_components (35.7.0)
            +    publishing_platform_publishing_components (35.8.0)
  ...
  describe "#validate_single_commit" do
    ...
    let(:commit_api_url) { "https://api.github.com/repos/publishing-platform/#{repo_name}/pulls/1/commits" }
  ...
  describe "#validate_ci_workflow_exists" do
    ...
    it "should raise an exception if no workflows are returned in the response" do
      ...
      expected_output = <<~MULTILINE_OUTPUT
        Error fetching CI workflow in API response for https://api.github.com/repos/publishing-platform/foo/actions/runs?head_sha=#{sha}
  ...
  describe "#approve!" do
    let(:approval_api_url) { "https://api.github.com/repos/publishing-platform/#{repo_name}/pulls/1/reviews" }
    ...
    it "should make an API call to approve the PR" do
      ...
      stub_request(:post, approval_api_url).with(
        body: {
          "event": "APPROVE",
          "body": "This PR has been scanned and automatically approved by [publishing-platform-dependabot-merger](https://github.com/publishing-platform/publishing-platform-dependabot-merger).\n",
        }.to_json,
  ...
  describe "#merge!" do
    it "should make an API call to merge the PR" do
      pr = PullRequest.new(pull_request_api_response)
      stub_request(:put, "https://api.github.com/repos/publishing-platform/#{repo_name}/pulls/1/merge").to_return(status: 200)

      pr.merge!
      expect(WebMock).to have_requested(:put, "https://api.github.com/repos/publishing-platform/#{repo_name}/pulls/1/merge")
    end
  end
  ...
  def stub_ci_endpoint(workflow_api_response)
    stub_request(:get, "https://api.github.com/repos/publishing-platform/#{repo_name}/actions/runs?head_sha=#{sha}")
  ...
  def stub_runs_endpoint(ci_workflow_id, ci_workflow_api_response)
    stub_request(:get, "https://api.github.com/repos/publishing-platform/#{repo_name}/actions/runs/#{ci_workflow_id}/jobs")

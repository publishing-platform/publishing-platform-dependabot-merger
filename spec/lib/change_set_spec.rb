...
RSpec.describe Dependency do
  describe "#internal?" do
    it "returns true if the dependency is owned by publishing-platform" do
      stub_request(:get, "https://rubygems.org/api/v1/gems/foo/owners.yaml")
        .to_return(
          body: <<~BODY,
            - id: 192816
              handle: publishing-platform
          BODY
        )
    ...
    it "returns false if the dependency is not owned by publishing-platform" do
      stub_request(:get, "https://rubygems.org/api/v1/gems/foo/owners.yaml")
        .to_return(
          body: <<~BODY,
            - id: 123
...
RSpec.describe ChangeSet do
  ...
  def multiple_dependencies_commit
    <<~TEXT
      Bump rack, rails and publishing_platform_sidekiq

      Bumps [rack](https://github.com/rack/rack), [rails](https://github.com/rails/rails) and [publishing_platform_sidekiq](https://github.com/publishing-platform/publishing_platform_sidekiq). These dependencies needed to be updated together.
      ...
      Updates `publishing_platform_sidekiq` from 5.7.0 to 5.8.0
      - [Changelog](https://github.com/publishing-platform/publishing_platform_sidekiq/blob/main/CHANGELOG.md)
      - [Commits](publishing-platform/publishing_platform_sidekiq@v5.7.0...v5.8.0)
      ...
      - dependency-name: rails
        dependency-type: direct:production
        update-type: version-update:semver-minor
      - dependency-name: publishing_platform_sidekiq
        dependency-type: direct:production
        update-type: version-update:semver-minor
      ...
  ...
  describe ".from_commit_message" do
    it "parses the commit message to discover the changed dependencies" do
      ...
      expect(change_set.changes).to eq([
        Change.new(Dependency.new("publishing_platform_publishing_components"), :minor),
      ])
    end
    ...
    it "supports commits that change more than one dependency" do
      ...
      expect(change_set.changes).to eq([
        ...
        Change.new(Dependency.new("publishing_platform_sidekiq"), :minor),
      ])
    end

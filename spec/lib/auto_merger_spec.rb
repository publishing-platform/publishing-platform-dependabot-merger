...
RSpec.describe AutoMerger do
  ...
  describe ".invoke_merge_script!" do
    it "should call `merge_dependabot_prs` with `dry_run: false`" do
  ...
  describe ".merge_dependabot_prs" do
    ...
    it "iterates through all Dependabot PRs of all Repos" do
      ...
      mock_repo_1 = instance_double("Repo", name: "Repo 1", publishing_platform_dependabot_merger_config: {}, dependabot_pull_requests: [mock_pr_1])
      mock_repo_2 = instance_double("Repo", name: "Repo 2", publishing_platform_dependabot_merger_config: {}, dependabot_pull_requests: [mock_pr_2, mock_pr_3])
    ...
    it "forwards the `dry_run` keyword arg if passed" do
      ...
      mock_repo = instance_double("Repo", name: "Repo", publishing_platform_dependabot_merger_config: {}, dependabot_pull_requests: [mock_pr])
    ...
    it "should make a call to PolicyManager.remote_config_exists?, which should block merge if false" do
      mock_repo = instance_double("Repo", name: "Repo 1", publishing_platform_dependabot_merger_config: {}, dependabot_pull_requests: [mock_pr])
      allow(Repo).to receive(:all).and_return([mock_repo])
      ...
      expect { AutoMerger.merge_dependabot_prs }.to output("Repo 1: the remote .publishing_platform_dependabot_merger.yml file is missing.\n").to_stdout
    end
    ...
    it "should make a call to PolicyManager.valid_remote_config_syntax?, which should block merge if false" do
      ...
      expect(AutoMerger).to_not receive(:merge_dependabot_pr)
      expect { AutoMerger.merge_dependabot_prs }.to output("Repo 1: the remote .publishing_platform_dependabot_merger.yml YAML syntax is corrupt.\n").to_stdout
    end
    ...
    it "should make a call to PolicyManager.remote_config_api_version_supported?, which should block merge if false" do
      ...
      expect(AutoMerger).to_not receive(:merge_dependabot_pr)
      expect { AutoMerger.merge_dependabot_prs }.to output("Repo 1: the remote .publishing_platform_dependabot_merger.yml file is using an unsupported API version.\n").to_stdout
    end

    it "should announce when no Dependabot PRs were found" do
      policy_manager # set up the mock policy manager object
      mock_repo = instance_double("Repo", name: "Repo 1", publishing_platform_dependabot_merger_config: {}, dependabot_pull_requests: [])
      allow(Repo).to receive(:all).and_return([mock_repo])
  ...
  describe ".analyse_dependabot_pr" do
    it "uses `merge_dependabot_pr` under the hood" do
      mock_pr = instance_double("PullRequest")
      mock_repo = instance_double("Repo", dependabot_pull_request: mock_pr, publishing_platform_dependabot_merger_config: {})
      mock_policy_manager = {}
      ...
      AutoMerger.analyse_dependabot_pr("https://github.com/publishing-platform/foo/pull/1234")
    end
  end
end
